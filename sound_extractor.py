...
#The program find interesting events in an audiofile based on changes in volume over time.
...
def get_spectrograms(y):
		...
		filtered_signal = librosa.feature.melspectrogram(S=unfiltered_signal, n_fft=N_FFT, hop_length=HOP_LENGTH,fmax=UPPER_HZ_LIMIT, fmin=LOWER_HZ_LIMIT, n_mels=N_MELS)
		tot_energy = librosa.feature.rmse(S=filtered_signal,frame_length=N_FFT, hop_length=HOP_LENGTH, center=True, pad_mode='reflect')
		return unfiltered_signal, raw_energy, filtered_signal, tot_energy

def segmentation(y,i,filename,mean_energy_of_file,CURRENT_POS,CLIP_DURATION,SAMPLE_RATE,file_duration,saved_bird_sounds,saved_noise,ignored_clips,ignored_files):
#Segmentation
...
def pre_processing_audio(CURRENT_POS,TOT_CLIPS,IGNORED_CLIPS,SAVED_NOISE,TOTAL_DURATION, IGNORED_FILES):
	for i in range(TOT_FILES_IMPORTED):
		...
		ignored_clips = 0
		ignored_files = 0 
		saved_bird_sounds,saved_noise,ignored_clips,ignored_files = segmentation(y,i,filename,mean_energy_of_file,CURRENT_POS,CLIP_DURATION,SAMPLE_RATE,file_duration,saved_bird_sounds,saved_noise,ignored_clips,ignored_files)
